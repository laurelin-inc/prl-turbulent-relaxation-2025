...
contains
   ...
   subroutine init_vpa_grid
      ...
      real :: del
      !> Variables for anisotropic temperature support
      integer :: is
      real, dimension(nspec) :: temp_parallel
      ...
      !> maxwell_vpa is the equilibrium Maxwellian in vpa
      !> Modified to support anisotropic temperatures (tpar for parallel direction)
      do is = 1, nspec
         if (spec(is)%use_anisotropic_temp) then
            temp_parallel(is) = spec(is)%temp_psi0 / spec(is)%tpar
         else
            temp_parallel(is) = spec(is)%temp_psi0 / spec(is)%temp
         end if
      end do
      maxwell_vpa = exp(-spread(vpa * vpa, 2, nspec) * spread(temp_parallel, 1, nvpa))
   ...
   subroutine init_mu_grid
      ...
      !> Variables for anisotropic temperature support
      integer :: is
      real, dimension(nspec) :: temp_perp
      ...
      !> maxwell_mu is the mu part of the v-space Maxwellian
      !> Modified to support anisotropic temperatures (tperp for perpendicular direction)
      do is = 1, nspec
         if (spec(is)%use_anisotropic_temp) then
            temp_perp(is) = spec(is)%temp_psi0 / spec(is)%tperp
         else
            temp_perp(is) = spec(is)%temp_psi0 / spec(is)%temp
         end if
      end do
      maxwell_mu = exp(-2.*spread(spread(spread(mu, 1, nalpha), 2, nztot) * spread(bmag, 3, nmu), 4, nspec) &
                       * spread(spread(spread(temp_perp, 1, nalpha), 2, nztot), 3, nmu))
